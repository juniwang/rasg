...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.ui = w.Mojo.ui || {};
    w.Mojo.ui.PageSelectorPanel = Mojo.Object.extend({
        init: function (id, options) {
            this._super(id, options);
            this.element().addClass("mojo-ui-page-selector-panel");
            this._children = [];
            this._panel = $('<div class="panel"></div>').appendTo(this.element());
            this._selector = $('<div class="selector"></div>').appendTo(this.element());
        ...
        _refresh: function () {
            var self = this;
            $('<div></div>').addClass("previous").html((this._options.current + 1) + "/" + (this._options.count < 0 ? this._children.length : this._options.count)).appendTo(this.element().find(".selector"));
            $('<div></div>').addClass("next").appendTo(this.element().find(".selector"));
            self._previousButton = new Mojo.ui.Button(undefined, {
                text: "",
                click: function () {
                    self.previous();
                }
            });
            self._previousButton.element().addClass("arrow").addClass("arrow-left").appendTo(this.element().find(".selector"));
            self._nextButton = new Mojo.ui.Button(undefined, {
                text: "",
                click: function () {
                    self.next();
                }
            });
            self._nextButton.element().addClass("arrow").addClass("arrow-right").appendTo(this.element().find(".selector"));
        },
        _refreshPage: function () {
            var self = this;
            this.element().find(".selector > .previous").html((this._options.current + 1) + "/" + (this._options.count < 0 ? this._children.length : this._options.count));
            if (this._options.onPageRefresh instanceof Function) {
                this._options.onPageRefresh(self);
            }
        },
        _setCurrent: function (current) {
            var self = this;
            this._options.current = current;
            this.element().find(".selector > .previous").html((current + 1) + "/" + (this._options.count < 0 ? this._children.length : this._options.count));
            this._previousButton.disable(false);
            this._nextButton.disable(false);
            this.element().find(".selector > .arrow-left").removeClass("arrow-left-disable").addClass("arrow-left-enable");
            this.element().find(".selector > .arrow-right").removeClass("arrow-right-disable").addClass("arrow-right-enable");
            if (current == 0) {
                self._previousButton.disable(true);
                this.element().find(".selector > .arrow-left").removeClass("arrow-left-enable").addClass("arrow-left-disable");
            }
            if (current == this._children.length - 1) {
                this._nextButton.disable(true);
                this.element().find(".selector > .arrow-right").removeClass("arrow-right-enable").addClass("arrow-right-disable");
            }
        },
        ...
        selector: function (current) {
            this._hideWait();
            if (current != this._options.current) {
                var self = this;
                if (self._timeoutHandle != undefined) {
                    clearTimeout(self._timeoutHandle);
                }
                if (self._options.beforeSelectFunc instanceof Function && self._options.beforeSelectFunc(self, current) === false) {
                    self._nextButton.disable(true);
                    self.element().find(".selector > .previous").html(Mojo.utils.locale('package', 'page_loading'));
                    self._timeoutHandle = setTimeout(function () {
                        self.selector(current)
                    }, 500);
                } else {
                    self._nextButton.disable(false);
                    if (current >= 0 && current < this._children.length) {
                        $.each(this._children, function (i, c) {
                            if (i == current) {
                                c.element().show();
                            } else if (i == self._options.current) {
                                c.element().hide();
                            }
                        });
                        this._setCurrent(current);
                        if (self._children.length > 0 && self._children.length < self._options.count) {
                            self._load();
                        }
                        if (this._options.selectFunc instanceof Function) {
                            this._options.selectFunc(this._options.current, this._children.length, this._options.count, this._options.pageSize);
                        }
                        if (this._hasLoaded === false && this._options.onLoaded instanceof Function) {
                            this._options.onLoaded(this._options.current, this._children.length, this._options.count, this._options.pageSize);
                            this._hasLoaded = true;
                        }
                    }
                }
        ...
        previous: function () {
            this.selector(this._options.current - 1);
        },
        next: function () {
            this.selector(this._options.current + 1);
        },
        _showWait: function () {
            this._panel.append('<div class="waiting"></div>');
        },
        _hideWait: function () {
            this._panel.find('.waiting').remove();
        },
        _load: function (params) {
            this._options.loadFunc(this._options.current, this._children.length, this._options.count, this._options.pageSize, params);
        },
        _getDefaultOptions: function () {
            var self = this;
            return {
        ...
        _addChild: function (child) {
            if (child != undefined && child != null) {
                this._panel.append(child.element().hide());
                this._children.push(child);
                this._refreshPage();
            }
        },
        _prependChild: function (child) {
            if (child != undefined && child != null) {
                this._panel.prepend(child.element().hide());
            ...
            if (data != undefined && data != null) {
                if (Array.isArray(data)) {
                    for (var i = data.length - 1; i >= 0; i--) {
                        var d = data[i];
                        self._prependChild(self._options.drawFunc(d));
                    }
                } else {
                    self._prependChild(self._options.drawFunc(data));
                }
            }
        },
        poppendData: function (data) {
            var self = this;
            if (data != undefined && data != null) {
                var popdata = this._children.pop();
                if (Array.isArray(data)) {
                    $.each(data, function (i, d) {
                        var adddata = self._options.drawFunc(d);
                        if (adddata != undefined && adddata != null) {
                            self._children.push(adddata);
                            popdata.element().before(adddata.element().hide());
                            self._refreshPage();
                        }
                    });
                } else {
                    var adddata = self._options.drawFunc(d);
                    if (adddata != undefined && adddata != null) {
                        this._children.push(adddata);
                        popdata.element().before(adddata.element().hide());
                        this._refreshPage();
                    }
                }
                if (popdata != undefined && popdata != null) {
                    this._children.push(popdata);
                }
            }
        },
        getSelectedObject: function (current) {
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.ui = w.Mojo.ui || {};
    w.Mojo.ui.PagePanel = w.Mojo.Object.extend({
        init: function (id, options) {
            this._super(id, options);
            this.element().addClass("mojo-ui-pagepanel");
            this._children = {
                _length: 0,
                _end: undefined
            };
            ...
            this._hasLoaded = false;
            this._panel = $('<div class="panel"></div>').appendTo(this.element());
            if (this._options.selector && this._options.selector.element() instanceof jQuery) {
                if (this._options.selectorPosition == "top") {
                    this._options.selector.element().prependTo(this.element());
                } else {
                    this._options.selector.element().appendTo(this.element());
                }
            }
            this._load();
        },
        _load: function () {
            this._showWait();
            this._options.loadFunc(this);
        },
        refreshSelector: function (data) {
            if (this._options.selector) {
                this._options.selector.refreshAll(data);
            }
        },
        selector: function (current) {
            if (this._options.beforeSelectFunc instanceof Function) {
                this._options.beforeSelectFunc(this, current);
            }
            if (current != this._current) {
                if (this._options.count > -1 && current >= this._options.count && this._children._end) {
                    current = "_end";
                }
                var child = this._children[current];
                var lastChild = this._children[this._current];
                if (lastChild != undefined && lastChild != null) {
                    this._children[this._current].element().hide();
                } else {
                    for (var ck in this._children) {
                        if (this._children[ck] && this._children[ck].element) {
                            this._children[ck].element().hide();
                        }
                    }
                }
                if (child != undefined && child != null) {
                    ...
                    if (this._options.selector && this._options.selector.selector) {
                        this._options.selector.selector(current);
                    }
                    if (this._hasLoaded === false && this._options.onLoaded instanceof Function) {
                        this._options.onLoaded(this._current);
                        this._hasLoaded = true;
                    }
                    if (this._options.selectFunc instanceof Function) {
                        this._options.selectFunc(current);
                    }
                } else {
                    this._showWait();
                    this._options.loadFunc(this, current);
                }
            }
        },
        isEnd: function (current) {
            if (current === undefined) {
                current = this._current;
            }
            if (current == '_end' || parseInt(current) >= this._options.count) {
                return true;
            }
            return false;
        },
        addData: function (data, start, append) {
            var self = this;
            if (data != undefined && data != null) {
                if (Array.isArray(data)) {
                    $.each(data, function (i, d) {
                        if (Mojo.utils.isNone(self._children[start]) == false) {
                            self._children[start].element().remove();
                        }
                        self._children[start] = self._options.drawFunc(this, d);
                        if (append === true) {
                            self._children._end = self._children[start];
                        }
                        self._panel.append(self._children[start].element().hide());
                        start++;
                        self._children._length++;
                    });
                } else {
                    if (Mojo.utils.isNone(self._children[start]) == false) {
                        self._children[start].element().remove();
                    }
                    self._children[start] = self._options.drawFunc(this, data);
                    if (append === true) {
                        self._children._end = self._children[start];
                    }
                    self._children._length++;
        ...
        removeData: function (index) {
            if (index instanceof Function) {
                var self = this;
                $.each(this._children, function (i, o) {
                    if (i != "_end" && i != "_length" && index(i, o) === true) {
                        self._children[i] = undefined;
                    }
                });
...
(function (w, $, undefined) {
    ...
    w.Mojo.lang['common'] = {
        ...
        'zh_cn': {
            ...
            'name': '昵称：',
            'need_logout': '重新登录',
            'need_logout_notice': '主公~您的账号在其它设备上登录了，本机登陆的账号自动下线，请重新登录~',
            'new': '新',
            'new_attack': '攻击：',
            'new_defence': '防御：',
    ...
    w.Mojo.lang['rob'] = {
        'zh_tw': {
            'No rob opponent has {{:collectfrag}}': '還沒有其他玩家持有【{{:collectfrag}}】～',
            'Retaliate': '反擊',
            'avoid war': '免戰',
            'avoid war status': '免戰狀態',
            'avoid war status content1': '主公，使用免戰牌後，1小時内免受進攻且不能出兵。（可中途撤銷）<br><b>免戰時間：{{:avoid_war_time}}</b>',
            'avoid war status content2': '主公，使用空城計後，12小時內免受進攻且不能出兵。（可中途撤銷）<br><b>免戰時間：{{:avoid_war_time}}</b>',
            'collect fail': '收寶失敗',
            'collect fragment info': '收集品資訊',
            'collect treasure': '收寶',
            'collection composite': '收取寶物',
            'collection detail': '寶物詳細',
            'composite': '合成',
            'composite content': '主公～合成寶物需要一小時時間！在這個過程中收集品有被奪走的危險，建議購買免戰牌，並及時收取寶物。<br><b>免戰時間：</b>{{:avoid_war_time}}<br><b>合成倒計時：</b>{{:countdown}}',
            'composite start': '合成開始',
            'compositing': '合成中',
            'count': '持有數：',
            'countdown': '合成倒數計時：{{:countdown}}',
            'frament not enough': '寶物收集品不足',
            'in avoid war': '免戰中',
            'refresh opps': '刷新對手',
            'reset avoid war time': '免戰時間重置',
            'reset avoid war time content': '主公～我軍現在還有免戰時間，如果使用【{{:name}}】，則免戰時間將重置。你確定要使用嗎？<br><b>免戰時間：{{:avoid_war_time}}</b>',
            'rob': '奪寶',
            'rob result preview': '奪寶結果預覽',
            'rob_tip': '小秘方：攻擊等級高的玩家奪寶成功率更高哦~',
            'wait in compositing': '正在合成請稍候',
            'you composite a new treasure, Congratulations': '恭喜主公，賀喜主公，【{{:name}}】合成成功！別忘了將寶物裝備到將領上哦～',
        },
        'zh_cn': {
            'No rob opponent has {{:collectfrag}}': '还没有其他玩家持有【{{:collectfrag}}】～',
            'Retaliate': '反击',
            'avoid war': '免战',
            'avoid war status': '免战状态',
            'avoid war status content1': '主公，使用免战牌后，1小时内免受进攻且不能出兵。（可中途撤销）<br><b>免战时间：{{:avoid_war_time}}</b>',
            'avoid war status content2': '主公，使用空城计后，12小时内免受进攻且不能出兵。（可中途撤销）<br><b>免战时间：{{:avoid_war_time}}</b>',
            'collect fail': '收宝失败',
            'collect fragment info': '收集品信息',
            'collect treasure': '收宝',
            'collection composite': '收取宝物',
            'collection detail': '宝物详细',
            'composite': '合成',
            'composite content': '主公～合成宝物需要一小时时间！在这个过程中收集品有被夺走的危险，建议购买免战牌，并及时收取宝物。<br><b>免战时间：</b>{{:avoid_war_time}}<br><b>合成倒计时：</b>{{:countdown}}',
            'composite start': '合成开始',
            'compositing': '合成中',
            'count': '持有数：',
            'countdown': '合成倒计时：{{:countdown}}',
            'frament not enough': '宝物收集品不足',
            'in avoid war': '免战中',
            'refresh opps': '刷新对手',
            'reset avoid war time': '免战时间重置',
            'reset avoid war time content': '主公～我军现在还有免战时间，如果使用【{{:name}}】，则免战时间将重置。你确定要使用吗？<br><b>免战时间：{{:avoid_war_time}}</b>',
            'rob': '夺宝',
            'rob result preview': '夺宝结果预览',
            'rob_tip': '小贴士：攻击等级高的玩家夺宝成功率更高哦~',
            'wait in compositing': '正在合成请稍候',
            'you composite a new treasure, Congratulations': '恭喜主公，贺喜主公，【{{:name}}】合成成功！别忘了将宝物装备到将领上哦～',
        },
    };
})(window, jQuery);;
(function (w, $, undefined) {
...
(function (w, $, undefined) {
    ...
    var g = w.Mojo.ajax = function (url, params, callback, onError, options) {
        ...
        if (true === Mojo.app.isNewClientVersion()) {
            Mojo.gap.ajax(Mojo.ajax.baseUrl + "/ajax" + url, params, function (data, textStatus, jqXHR) {
                if (options && options.showWait === true) {
                ...
                if (status == 493) {
                    Mojo.app.toast.show(Mojo.utils.locale('common', 'bad request'));
                }
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.FightProfile = w.Mojo.Object.extend({
        clsname: function () {
            return "com.FightProfile";
        },
        init: function (options) {
            this._super('com-profile-fight', options);
            this.element().addClass('mojo-com-profile-fight');
            ...
            this.element().click(function () {
                (new Mojo.com.FightProfileDialog({
        ...
        _addRight: function () {
            this._attack = new Mojo.ui.Label(undefined, {
                classes: ['attack'],
                text: '0',
            });
            this._bse = new Mojo.ui.Progress(undefined, {
                classes: ['bse'],
                labelTemplate: '#{divide}',
            });
            ...
            this.element().find('.right').append(this._attack.element()).append(this._bse.element()).append($('<div class="bse-border"></div')).append(this._sp.element()).append($('<div class="sp-border"></div>'));
        },
        data: function (data) {
            if (data == undefined) {
                return this._data;
            }
            this._data = $.extend(true, this._data, data);
            this._refresh();
        },
        _refresh: function () {
            this._name.html(this._data.name);
            this._level.html(this._data.level);
            if (parseInt(this._data.xp) > parseInt(this._data.next_xp)) {
                this._data.xp = this._data.next_xp;
            }
            this._xp.value({
                value: this._data.xp,
                max: this._data.next_xp
            });
            this._attack.text(this._data.attack_min + '～' + this._data.attack_max);
            this._bse.value({
                value: this._data.total_bse,
                max: this._data.total_bs
            });
            this._sp.value({
                value: this._data.sp,
                max: this._data.stamina
            });
            this._options.refreshCallback(this._data);
        },
        spRefresh: function (sp) {
            this._data.sp = sp;
            this._sp.value({
                value: this._data.sp,
                max: this._data.stamina
        ...
        sync: function () {
            var self = this;
            Mojo.ajax('/battle/profile', {}, function (result) {
                if (result.errorCode == 0) {
        ...
        _calcAvoidWarTime: function () {
            if (this._data && this._data.avoid_war_time > 0) {
                this._data.avoid_war_time--;
            }
        },
        reset: function () {
            this._data.avoid_war_time = 0;
        },
    });
})(window, jQuery);;
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.FightProfileDialog = w.Mojo.ui.Dialog.extend({
        clsname: function () {
            return "com.FightProfileDialog";
        },
        init: function (options) {
            this._super('com-profile-fight-dialog', options);
            this.element().addClass('mojo-com-profile-fight-dialog');
            var self = this;
            this._interval = w.setInterval(function () {
                self._refresh();
            }, 1000);
        ...
        _getDefaultOptions: function () {
            return $.extend(true, this._super(), {
                dataProvider: $.noop,
                title: Mojo.utils.locale('ui', 'Fight Informations'),
            });
        },
        _refresh: function () {
        ...
        _genTooltip: function (data) {
            ...
            new Mojo.ui.Label(undefined, {
                classes: ['simple_attack'],
                text: Mojo.utils.locale('common', 'team_attack') + data.attack_min + "~" + data.attack_max
            }).element().appendTo(this._tips);
            new Mojo.ui.Label(undefined, {
                classes: ['simple_defence'],
                text: Mojo.utils.locale('common', 'team_defence') + data.defence_min + "~" + data.defence_max
            }).element().appendTo(this._tips);
            new Mojo.ui.Label(undefined, {
                classes: ['general_count'],
                text: Mojo.utils.locale('common', 'general_count') + data.total_bse + "/" + data.total_bs
            }).element().appendTo(this._tips);
            new Mojo.ui.Label(undefined, {
                classes: ['sp'],
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.CorpsProfile = w.Mojo.Object.extend({
        clsname: function () {
            return "com.CorpsProfile";
        },
        init: function (data, options) {
            this._data = data || {};
            this._super('com-profile-corps', options);
            this.element().addClass('mojo-com-profile-corps');
            this._attackLabel = new Mojo.ui.Label(undefined, {
                text: (this._data.attack_min ? this._data.attack_min : "0") + "~" + (this._data.attack_max ? this._data.attack_max : "0")
            });
            this._defenceLabel = new Mojo.ui.Label(undefined, {
                text: (this._data.defence_min ? this._data.defence_min : "0") + "~" + (this._data.defence_max ? this._data.defence_max : "0")
            });
            this.element().append(this._attackLabel.element().addClass('corps-attack'));
            this.element().append(this._defenceLabel.element().addClass('corps-defence'));
        },
        _refresh: function () {
            var buff_attack_add = '';
            var buff_defence_add = '';
            if (this._data.buff_attack_add != undefined && this._data.buff_attack_add != '') buff_attack_add = '<span class="text-red">' + "(+" + this._data.buff_attack_add + "%)" + '</span>';
            if (this._data.buff_defence_add != undefined && this._data.buff_defence_add != '') buff_defence_add = '<span class="text-red">' + "(+" + this._data.buff_defence_add + "%)" + '</span>';
            this._attackLabel.text((this._data.attack_min ? this._data.attack_min : "0") + "~" + (this._data.attack_max ? this._data.attack_max : "0") + buff_attack_add);
            this._defenceLabel.text((this._data.defence_min ? this._data.defence_min : "0") + "~" + (this._data.defence_max ? this._data.defence_max : "0") + buff_defence_add);
        },
        data: function (data) {
            if (data === undefined) {
                return this._data;
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.Opponent = w.Mojo.Object.extend({
        clsname: function () {
            return "com.Opponent";
        },
        init: function (data, options) {
            this._data = data;
            this._super(undefined, options);
            this.element().addClass('mojo-com-opponent').addClass('box-inner');
            this._addAttributes();
        },
        _addAttributes: function () {
            ...
            this.element().append('<div class="level">' + Mojo.utils.locale('common', 'lv') + this._data.level + '</div>').append('<div class="name">' + name + '</div>').append('<hr>');
            if (this._data.force != undefined && this._data.force.name != '') {
                this.element().append('<div class="fname">' + Mojo.utils.locale('common', 'force') + this._data.force.name + '</div>');
            } else {
                this.element().append('<div class="fname force-none">' + Mojo.utils.locale('battle', 'force_none') + '</div>');
            }
            this.element().append('<div class="attack">' + Mojo.utils.locale('common', 'new_attack') + attack + '</div>').append('<div class="defence">' + Mojo.utils.locale('common', 'new_defence') + '?' + '</div>');
            if (self._data.avoid_war_time != undefined) {
                if (self._data.avoid_war_time <= 0) {
                    self.attackBtn = new Mojo.ui.Button(null, {
                        special: 'go-attack-btn',
                        classes: ['go-button'],
                        text: self._options.goBtnText,
                        click: function () {
                            self._options.clickFunc(self._data);
                        },
                    });
                    this.element().append(self.attackBtn.element());
                } else {
                    self._avoidWarStatus = $('<div class="inAvoidWar">' + self.getL("battle", "in_avoid_war") + '</div>').appendTo(this.element());
                }
            } else {
                self.attackBtn = new Mojo.ui.Button(null, {
                    special: 'go-attack-btn',
        ...
        _addButtons: function () {
            ...
            if (this._options.button1 != undefined) {
                this._btn1 = new Mojo.ui.Button(undefined, {
                    ...
                    click: function () {
                        self._options.clickButton1(self._data);
                    },
                });
                this.element().find('.buttons').append(this._btn1.element());
            }
            if (this._options.button2 != undefined) {
                this._btn2 = new Mojo.ui.Button(undefined, {
                    classes: ['go-button'],
                    text: this._options.button2,
                    click: function () {
                        self._options.clickButton2(self._data);
                    },
                });
                this.element().find('.buttons').append(this._btn2.element());
            }
        },
        _getDefaultOptions: function () {
            return {
                goBtnText: Mojo.utils.locale('battle', 'action'),
                button1: undefined,
                clickButton1: $.noop,
                clickFunc: $.noop,
            }
...
(function (w, $, undefined) {
    ...
    w.Mojo.com.BattlePreviewDialog = w.Mojo.ui.Dialog.extend({
        ...
        _addContent: function () {
            ...
            _oppName = $('<div class="opp_name"></div>').html(self._data.opponent_name).appendTo(_opp);
            if (self._data.info == null || self._data.info == undefined) {
                _ifwin = $('<div class="ifwin"></div>').appendTo(this._content);
                _win = $('<div class="win"></div>').html(Mojo.utils.locale('common', 'when win')).appendTo(_ifwin);
                _ifwin.append((new Mojo.ui.Label(undefined, {
                    classes: ['winxp'],
                ...
                if (self._data.entity_rate != null && self._data.entity_rate != undefined && self._data.entity_rate != '') {
                    _rate = $('<div class="rate"></div>').html(self._data.entity_rate).appendTo(_ifwin);
                } else if (!self._options.pk_mode && self._options.entity_id == '') {
                    _rate = $('<div class="rate"></div>').html(Mojo.utils.locale('battle', 'win_message_06')).appendTo(_ifwin);
                }
            } else {
                _resultInfo = $('<div class="info"></div>').html(self._data.info).appendTo(this._content);
            }
        },
        _addHandleButtons: function () {
            var self = this;
            if (self._data.info == null || self._data.info == undefined) {
                _attackBtn = new Mojo.ui.Button('attack-btn', {
                    special: 'button-big-red',
                    text: self._options.atackBtnText,
                    click: function () {
                        _attackBtn.disable(true);
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    var g = w.Mojo.com.EntityDetail = function (parent, options) {
        this._options = {
            id: 0,
            pid: 0,
            selectClick: $.noop,
            cancelClick: $.noop,
            entityDetailClass: 'mojo-com-entity-detail',
        };
        $.extend(true, this._options, options);
        var self = this;
        var id = "";
        if (this._options.id instanceof Object) {
            id = this._options.id.entity_id;
        } else {
            id = this._options.id;
        }
        this._element = $(this._template.replace(/#\{id\}/g, id));
        this._parent = parent;
    ...
    g.prototype = {
        ...
        getButtonPanel: function () {
            return this.element().find(".detail-panel");
        },
        addButtonGroup: function (buttonGroup) {
            this.element().find(".detail-panel").append(buttonGroup);
        },
        _refresh: function () {
            var self = this;
            new Mojo.com.Card(self._options.id, {
                pid: self._options.pid,
                top: 0,
                left: 0
            }, function (that) {
                self._element.find('.card').html('').append(that.element());
                self._element.find('.introduction').html('简介：<br>' + that.getAttr("description"));
                var s = '';
                var skills = that.getAttr("skills");
                if (skills != undefined) {
                    for (var i in skills) {
                        var skill = skills[i];
                        s += skill.description + '<br>';
                    }
                    self._element.find('.effect').html(s);
                }
            });
        },
        _template: '<div id="entity_detail_#{id}" class="mojo-com-entity-detail">\
   <div class="card"></div>\
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.CollectList = w.Mojo.Object.extend({
        clsname: function () {
            return "com.CollectList";
        },
        init: function (options) {
            this._super('com-collect-list', options);
            this._hasLoaded = false;
            this._collects = {};
            ...
            this._list = new Mojo.ui.ListPanel(undefined, {
                ...
                loadFunc: function (start, count) {
                    Mojo.ajax('/collect/simple', {}, function (result) {
                        if (result.errorCode == 0) {
                            if (result.data.length < 4) {
                                for (; result.data.length < 4;) {
                                    result.data.push(undefined);
                                }
                            } else if (self._options.showUnlock === true) {
                                result.data.push(undefined);
                            }
                            self._list.element().width(result.data.length * self._options.unitWidth);
                            self._list.appendData(result.data);
                            self._scroll.refresh();
                        } else {
                ...
                drawFunc: function (data) {
                    return self._getCollectElement(data);
                },
            ...
            Mojo.app.scroll = this._scroll = new Mojo.ui.Scroll(undefined, this._list, {
                direction: 1,
        ...
        _getCollectElement: function (data) {
            var obj = new Mojo.Object(undefined, {
                classes: ['mojo-com-collectlist-element', 'image'],
            });
            if (data == undefined) {
                obj.element().append('<div class="locked"></div>');
            } else {
                var collectId = "collect_" + data.id;
                this._collects[collectId] = obj;
            ...
            obj.element().click(function () {
                ...
                if (self._options.onSelect instanceof Function) {
                    self._options.onSelect(self, obj.element().index());
                }
            });
            return obj;
        },
        selector: function (current) {
            ...
            $.each(this.element().find(".mojo-com-collectlist-element"), function (i, e) {
                if (current == i) {
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.CollectFragment = w.Mojo.Object.extend({
        clsname: function () {
            return "com.CollectFragment";
        },
        init: function (data, options) {
            this._data = data;
            this._super(undefined, options);
            this.element().addClass('mojo-com-collect-fragment');
            this.element().append('<div class="image"></div>');
            var self = this;
            var refreshCount = function (entity_id, entity_valid, entity_img) {
                if (entity_id != '') {
                    if (entity_valid == 1) {
                        self._data.count = parseInt(self._data.count) + 1;
                        self._data.medium_image = entity_img;
                        self._refresh();
                    }
                }
                self._options.refreshCompositeBtn();
            };
            this.element().click(function () {
                Mojo.track.onEvent('07_011');
                (new Mojo.com.CollectFragmentDialog(self._data, {
                    fightProfile: self._options.fightProfile,
                    doRob: function (entity_id, entity_name) {
                        self._options.doRob(entity_id, entity_name, refreshCount);
                    },
                })).open();
            });
            this._refresh();
        ...
        _refresh: function () {
            var border = this.element().find('.image');
            border.empty();
            if (this._data.count == 0) {
                border.append('<img class="img-disabled-class" src="' + this._data.medium_image + '">');
            } else {
                border.append('<img src="' + this._data.medium_image + '"><div class="count">' + this._data.count + '</div>');
            }
        },
        data: function (value) {
            if (value == undefined) {
        ...
        _refreshCount: function (count) {
            ...
            if (count == 0) {
                ...
                border.append('<img src="' + this._data.medium_image + '"><div class="count">' + count + '</div>');
            }
        },
        _getDefaultOptions: function () {
            return $.extend(true, this._super(), {
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.CollectFragmentDialog = w.Mojo.ui.Dialog.extend({
        clsname: function () {
            return "com.CollectFragmentDialog";
        },
        init: function (data, options) {
            this._data = data;
            this._super(undefined, options);
            this.element().addClass('mojo-com-collectfragdlg');
            this._addContent();
            this._addHandleButtons();
        },
        _addContent: function () {
            if (this._data != null && this._data != undefined) {
                this._entity = new Mojo.com.LargeEntity($.extend(true, this._data, {
                    level: 1,
                    type_id: 6,
                    group_id_1: ''
                }), {
                ...
                this._content.append($('<div class="frag-count">' + Mojo.utils.locale('rob', 'count') + this._data.count + '</div>'))
            }
        },
        _addHandleButtons: function () {
            var self = this;
            this._footer.append((new Mojo.ui.Button('rob-btn', {
                special: 'button-big-red',
                text: Mojo.utils.locale('rob', 'rob'),
                click: function () {
                    self.close();
                    self._options.doRob(self._data.id, self._data.name);
                },
            })).element());
            this._footer.append((new Mojo.ui.Button('close-btn', {
                text: Mojo.utils.locale('common', 'close'),
        ...
        _getDefaultOptions: function () {
            return $.extend(true, this._super(), {
                title: Mojo.utils.locale('rob', 'collect fragment info'),
                handle: $.noop,
                fightProfile: null,
                doRob: $.noop,
            });
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.CollectionDialog = w.Mojo.ui.Dialog.extend({
        clsname: function () {
            return "com.CollectionDialog";
        },
        init: function (data, options) {
            this._data = data;
            this._super(undefined, options);
            this.element().addClass('mojo-com-collectiondlg');
        ...
        _addContent: function () {
            var self = this;
            if (self._options.view_mode) {
                self._title.html(Mojo.utils.locale('rob', 'collection detail'));
            }
            var data = null;
            if (this._data != null && this._data != undefined) {
                this._entity = new Mojo.com.LargeEntity(this._data, {
                    showSkill: (self._options.view_mode == true)
                });
                this._content.append(this._entity.element());
            }
            this._content.append('<div class="tip"></div>');
            if (self._options.view_mode == false) {
                this._content.find('.tip').html(Mojo.utils.locale('rob', 'you composite a new treasure, Congratulations', {
        ...
        _addHandleButtons: function () {
            ...
            if (self._options.view_mode == false) {
                ...
                var platformBtn = new Mojo.com.PlatformButton(undefined, self._options);
                ...
                if (self._options.can_composite) {
                    this._footer.append((new Mojo.ui.Button('composite-btn', {
                        special: 'button-big-red',
                        text: Mojo.utils.locale('rob', 'composite'),
                        click: function () {
                            Mojo.ajax('/collect/composite', {
                                id: self._data.id
                            }, function (result) {
                                if (result.errorCode == 0) {
                                    self._data.count = result.data.count;
                                    self._content.find('.tip').html(Mojo.utils.locale('rob', 'you composite a new treasure, Congratulations', {
                                        name: result.data.name
                                    }));
                                    self._footer.find('#composite-btn').remove();
                                    self._options.btnClick = function () {
                                        self.close();
                                    };
                                    var platformBtn = new Mojo.com.PlatformButton(result.event, self._options);
                                    self._footer.prepend(platformBtn.element());
                                    self._options.refreshCountFunc(result.data);
                                } else {
                                    (new Mojo.com.TipsDialog({
                                        title: Mojo.utils.locale('common', 'tips'),
                                        message: result.errorMsg,
        ...
        _getDefaultOptions: function () {
            return $.extend(true, this._super(), {
                title: Mojo.utils.locale('rob', 'collection composite'),
                handle: $.noop,
                view_mode: false,
                can_composite: false,
                access_token: null,
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.Collection = w.Mojo.Object.extend({
        clsname: function () {
        ...
        init: function (data, options) {
            this._data = data;
            this._super(undefined, options);
            this.element().addClass('mojo-com-collection');
            this.element().append('<div class="treasure"></div><div class="fragments"></div>');
            this._initData();
            this._addTopElements();
            this._arrangeFragments();
            this._addButtons();
        },
        _initData: function () {
            this._compositeTime = this._data.away_time;
            this._avoidWarTime = this._options.avoidWarTime;
        },
        _addTopElements: function () {
            var self = this;
            if (this._data.count > 0) {
                this.element().find('.treasure').append('<div class="image"><img src="' + this._data.medium_image + '"><div class="count">' + this._data.count + '</div></div>');
            } else {
                this.element().find('.treasure').append('<div class="image"><img class="img-disabled-class" src="' + this._data.medium_image + '"></div>');
            }
            this.element().find('.treasure').append('<div class="name">' + this._data.name + '</div>').click(function () {
                Mojo.track.onEvent('07_051');
                (new Mojo.com.CollectionDialog(self._data, {
                    view_mode: true,
                    can_composite: false,
                    access_token: null,
                    defaultText: '',
                    appendix: '',
                    refreshCountFunc: function (data) {
                        self._refreshCount(data);
                    },
                })).open();
            });
        },
        _arrangeFragments: function () {
            this._fragments = [];
            var self = this;
            for (var i = 0; i < this._data.fragments.length; i++) {
                var f = new Mojo.com.CollectFragment(this._data.fragments[i], {
                    fightProfile: self._options.fightProfile,
                    doRob: function (entity_id, entity_name, refreshCount) {
                        self._options.doRob(entity_id, entity_name, refreshCount, function (time) {
                            self._avoidWarTime = time;
                            self._avoidwarBtn.text(Mojo.utils.locale('rob', 'avoid war'));
                            self._setAvoidwarInterval();
                        });
                    },
                    refreshCompositeBtn: function () {
                        self._refresh();
                    },
                });
                this.element().find('.fragments').append(f.element());
                var positionIndex = self._options.layouts[self._data.fragments.length][i];
                f.element().css('left', self._options.offsets[positionIndex].x).css('top', self._options.offsets[positionIndex].y);
                this._fragments.push(f);
            }
        },
        _addButtons: function () {
            var self = this;
            this._compositeStartBtn = new Mojo.ui.Button(undefined, {
                special: 'button-big-red',
                text: Mojo.utils.locale('rob', 'composite'),
                classes: ['composite-btn'],
                click: function () {
                    Mojo.ajax('/collect/compositeStart', {
                        id: self._data.id
                    }, function (result) {
                        if (result.errorCode == 0) {
                            Mojo.track.onEvent('07_061');
                            self._compositeTime = result.data.time;
                            self._refresh();
                            (new Mojo.com.CompositeDialog(result.data, {
                                callback: function (time) {
                                    self._avoidWarTime = time;
                                    self._avoidwarBtn.text(Mojo.utils.locale('rob', 'in avoid war'));
                                    self._setAvoidwarInterval();
                                },
                            })).open(true);
                        } else {
            ...
            this.element().append(this._compositeStartBtn.element());
            this._btn = new Mojo.ui.Button(undefined, {
                special: 'button-big-red',
                text: Mojo.utils.locale('rob', 'collect treasure'),
                classes: ['composite-btn'],
                click: function () {
                    Mojo.ajax('/collect/composite', {
                        id: self._data.id
                        ...
                        if (result.errorCode == 0) {
                            Mojo.track.onEvent('07_062');
                            self._data.count = result.data.count;
                            self._compositeTime = undefined;
                            var weiboPara = {
                                access_token: null,
                                defaultText: '',
                                appendix: '',
                            ...
                            if (result.event != undefined && result.event != null) {
                                for (index in result.event) {
                                    if (result.event[index].name == 'weibo.create') {
                                        weiboPara.access_token = result.event[index].data.access_token;
                                        weiboPara.defaultText = result.event[index].data.content;
                                        weiboPara.appendix = result.event[index].data.download_url;
                                        weiboPara.expire = result.event[index].expire;
                                    }
                                }
                            }
                            setTimeout(function () {
                                (new Mojo.com.CollectionDialog(self._data, weiboPara)).open();
                            }, 400);
                            self._refreshCount(result.data);
                            ...
                            dlg.open();
                        } else if (result.errorCode == 50004) {
                            var dialog = new Mojo.ui.Dialog(undefined, {
                                title: Mojo.utils.locale('rob', 'collect fail'),
                                content: result.errorMsg,
                            });
                            (new Mojo.ui.Button(undefined, {
                                text: Mojo.utils.locale('rob', 'Retaliate'),
                                click: function () {
                                    dialog.close();
                                    Mojo.app.redirect('/message', {
                                        selected: 1
                                    });
                                },
                            })).element().appendTo(dialog._footer);
                            (new Mojo.ui.Button(undefined, {
                                text: Mojo.utils.locale('common', 'close'),
                                click: function () {
                                    dialog.close();
            ...
            this.element().append(this._btn.element());
            this._avoidwarBtn = new Mojo.ui.Button(undefined, {
                text: ((self._avoidWarTime != undefined && self._avoidWarTime > 0) ? Mojo.utils.locale('rob', 'in avoid war') : Mojo.utils.locale('rob', 'avoid war')),
                click: function () {
                    Mojo.ajax('/collect/avoidWar', {}, function (result) {
                        if (result.errorCode == 0) {
                            (new Mojo.com.AvoidWarStatusDialog(result.data, {
                                avoidWarTime: (result.data.avoid_war_time == undefined ? self._avoidWarTime : parseInt(result.data.avoid_war_time)),
                                callback: function (time) {
                                    self._avoidWarTime = time;
                                    self._avoidwarBtn.text(Mojo.utils.locale('rob', 'in avoid war'));
                                    self._setAvoidwarInterval();
                                    self._options.callback(time);
                                },
                            })).open(true);
                        } else {
                            (new Mojo.com.TipsDialog({
                ...
                classes: ['avoidwar-btn'],
            });
            this.element().append(this._avoidwarBtn.element());
            this._countdown = new Mojo.ui.Label(undefined, {
                text: '',
                classes: ['countdown'],
            });
            this.element().append(this._countdown.element());
            this._refresh();
        },
        _refreshCount: function (data) {
            var self = this;
            var border = this.element().find('.treasure > .image');
            border.empty();
            if (data.count == 0) {
                border.append('<img class="img-disabled-class" src="' + this._data.medium_image + '">');
            } else {
                border.append('<img src="' + this._data.medium_image + '"><div class="count">' + data.count + '</div>');
            }
            if (self._fragments != null && self._fragments != undefined) {
                for (var i = 0; i < self._fragments.length; i++) {
                    for (var j = 0; j < data.fragments.length; j++) {
                        if (self._fragments[i].data().id == data.fragments[j].id) {
                            self._fragments[i]._refreshCount(data.fragments[j].count);
                        }
                    }
                }
                self._refresh();
        ...
        _refresh: function () {
            var self = this;
            if (self._isInCompositing()) {
                this._compositeStartBtn.show();
                this._compositeStartBtn.disable(true);
                this._compositeStartBtn.text(Mojo.utils.locale('rob', 'compositing'));
                this._compositeStartBtn._options.disableClick = function () {
                    Mojo.app.toast.show(Mojo.utils.locale('rob', 'wait in compositing'));
                };
                this._btn.hide();
                this._countdown.text(Mojo.utils.locale('rob', 'countdown', {
                    countdown: Mojo.utils.formatTime(this._compositeTime)
                }));
                this._countdown.show();
                this._interval = w.setInterval(function () {
                    self._refreshCompositeTime();
                }, 1000);
            } else if (self._isCompositComplete()) {
                this._countdown.hide();
                this._btn.show();
                this._compositeStartBtn.hide();
            } else {
                this._countdown.hide();
                this._btn.hide();
                this._compositeStartBtn.show();
                this._compositeStartBtn.text(Mojo.utils.locale('rob', 'composite'));
                this._compositeStartBtn._options.disableClick = function () {
                    Mojo.app.toast.show(Mojo.utils.locale('rob', 'frament not enough'));
                };
                if (self._isCompositeCanStart()) {
                    this._compositeStartBtn.disable(false);
                } else {
                    this._compositeStartBtn.disable(true);
                }
            }
            if (this._compositeStartBtn.disable == false) {
                this._compositeStartBtn.element().addClass('blink-class');
            } else {
                this._compositeStartBtn.element().removeClass('blink-class');
            }
            this._setAvoidwarInterval();
        },
        _setAvoidwarInterval: function () {
            var self = this;
            w.clearInterval(this._avoidwarInterval);
            if (this._avoidWarTime > 0) {
                this._avoidwarBtn.text(Mojo.utils.locale('rob', 'in avoid war'));
                this._avoidwarInterval = w.setInterval(function () {
                    self._refreshAvoidWarTime();
                }, 1000);
            } else {
                this._avoidwarBtn.text(Mojo.utils.locale('rob', 'avoid war'));
            }
        },
        _refreshCompositeTime: function () {
            if (this._compositeTime > 0) {
                this._compositeTime--;
                this._countdown.text(Mojo.utils.locale('rob', 'countdown', {
                    countdown: Mojo.utils.formatTime(this._compositeTime)
                }));
        ...
        _isCompositeCanStart: function () {
            if (!this._isInCompositing() && !this._isCompositComplete()) {
                for (var i = 0; i < this._fragments.length; i++) {
                    if (this._fragments[i].data().count <= 0) {
                        return false;
                    }
                }
                return true;
        ...
        _isInCompositing: function () {
            if (this._compositeTime != undefined && this._compositeTime != 0) {
                return true;
            }
            return false;
        },
        _isCompositComplete: function () {
            if (this._compositeTime != undefined && this._compositeTime == 0) {
                return true;
            }
            return false;
        },
        _canComposited: function () {
            for (var i = 0; i < this._fragments.length; i++) {
                if (this._fragments[i].data().count <= 0) {
                    return false;
                }
            }
            if (this._compositeTime == undefined) {
                return true;
            } else {
                return false;
            }
        ...
        _getDefaultOptions: function () {
            ...
            return $.extend(true, this._super(), {
                ...
                doRob: $.noop,
                callback: $.noop,
                treasurePos: self.getR("treasurePos"),
                btnPos: {
                    x: 200,
                    y: 385
                },
                offsets: self.getR("offsets"),
                fragments: [],
                layouts: [
                    [],
                    [],
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.CompositeDialog = w.Mojo.ui.Dialog.extend({
        clsname: function () {
            return "com.CompositeDialog";
        },
        init: function (data, options) {
            this._data = data;
            this._super(undefined, options);
            this.element().addClass('mojo-com-compositedlg');
            var self = this;
            this._message = $('<div class="paragraph"></div>').appendTo(this._content);
            _goods = $('<div class="goods"></div>').appendTo(this._content);
            _card = $('<div class="card"></div>').appendTo(_goods);
            ...
            _priceValue = $('<div class="price-value"></div>').appendTo(_price);
            if (this._data.props.goods_vm > 0) {
                _priceIcon.addClass('vm');
                _priceValue.html(this._data.props.goods_vm);
            } else {
                _priceIcon.addClass('rm');
                _priceValue.html(this._data.props.goods_rm);
            }
            this._compositeTime = this._data.time;
            this._interval = w.setInterval(function () {
                self._calcCompositeTime();
            }, 1000);
            this._avoidWarTime = this._data.avoid_war_time;
            this._avoidWarInterval = w.setInterval(function () {
                if (self._avoidWarTime > 0) {
                    self._avoidWarTime--;
                    if (self._avoidWarDialog != undefined && self._avoidWarDialog != null) {
                        self._avoidWarDialog.element().find('.content').html(Mojo.utils.locale('rob', 'reset avoid war time content', {
                            name: self._data.props.goods_name,
                            avoid_war_time: Mojo.utils.formatTime(self._avoidWarTime)
                        }));
                    }
                }
            }, 1000);
        ...
        _refresh: function () {
            this._count.html(this._data.props.count);
            this._message.html(Mojo.utils.locale('rob', 'composite content', {
                avoid_war_time: Mojo.utils.formatTime(this._avoidWarTime),
                countdown: Mojo.utils.formatTime(this._compositeTime)
            }));
        },
        _calcCompositeTime: function () {
            if (this._compositeTime != undefined && this._compositeTime > 0) {
                this._compositeTime--;
                this._message.html(Mojo.utils.locale('rob', 'composite content', {
                    avoid_war_time: Mojo.utils.formatTime(this._avoidWarTime),
        ...
        _addBuyBtn: function () {
            var self = this;
            this._footer.prepend((new Mojo.ui.Button('buy-btn', {
                special: 'button-big-red',
                text: Mojo.utils.locale('common', 'buy'),
                click: function () {
                    Mojo.ajax('/mall/Buy', {
                        ...
                        if (result.errorCode == 0) {
                            self._data.props.count = self._data.props.count + 1;
                            self._data.props.id = result.data.entities[0].player_entity_id;
                            self._refresh();
                            self.element().find('#buy-btn').remove();
                            self._addUseBtn();
                        } else {
                            (new Mojo.com.BuyFailDialog({
                                useRm: (self._data.props.goods_rm > 0 ? true : false),
                                message: result.errorMsg,
                            })).open(true);
                        }
                    }, function () {}, {
        ...
        _addUseBtn: function () {
            var self = this;
            this._footer.prepend((new Mojo.ui.Button('use-btn', {
                special: 'button-big-red',
                text: Mojo.utils.locale('common', 'use'),
                click: function () {
                    if (self._data.avoid_war_time > 0) {
                        w.clearInterval(self._interval);
                        self.close();
                        (new Mojo.com.ResetAvoidWarDialog(self._data.props, {
                            avoidWarTime: self._avoidWarTime,
                            message: Mojo.utils.locale('rob', 'reset avoid war time content', {
                                name: self._data.props.goods_name,
                                avoid_war_time: Mojo.utils.formatTime(self._avoidWarTime)
                            }),
                            callback: function (time) {
                    ...
                    Mojo.ajax('/entity/Use', {
                        ...
                        if (result.errorCode == 0) {
                            ...
                            self._refresh();
                            self.close();
                            Mojo.app.toast.show(Mojo.utils.locale('common', 'reset_avoid_war', {
                                avoid_war_time: Mojo.utils.formatTime(result.data.value)
                            }));
                            self._options.callback(result.data.value);
                        } else {
        ...
        _getDefaultOptions: function () {
            var self = this;
            return $.extend(true, this._super(), {
                title: Mojo.utils.locale('rob', 'composite start'),
                message: '',
                callback: $.noop,
                close: function () {
                    w.clearInterval(self._interval);
...
(function (w, $, undefined) {
    ...
    w.Mojo.page.Rob = w.Mojo.Page.extend({
        clsname: function () {
            return "page.Rob";
        },
        init: function (entity, opponent, msgId) {
            this._super('page-rob', {
                fightProfile: true,
            });
            this._msgId = Mojo.utils.getFromParams('messageId');
            this._entityId = Mojo.utils.getFromParams('entityId');
            this._opponentId = Mojo.utils.getFromParams('opponentId');
            this._tutorialStep = 0;
            this._hasTutorial = false;
            this._tutorialData = undefined;
            this._index = 0;
            this._collectionlist = new Array(12);
        },
        _getTutorialStep: function (response) {
            if (response && response.event != undefined && response.event != null) {
                for (var index in response.event) {
                    var evt = response.event[index];
                    if (evt.name == "tutorial_start") {
                        this._tutorialStep = evt.data.step;
                    }
                }
            }
        },
        load: function () {
            var self = this;
            this._super();
            this.fightProfile.hide();
            this._corpsProfile = new Mojo.com.CorpsProfile();
            this.element().append(this._corpsProfile.element());
            this._corpsProfile.element().click(function () {});
            if (self._msgId == undefined || self._msgId == '') {
                this._refresh();
            } else {
                Mojo.ajax('/battle/render', {
                    type: 'rob',
                    ...
                    if (result.errorCode == 0) {
                        self._entity = result.data.data.entity, self._opponent = result.data.data.opponent, self._refresh();
                    }
                }, function () {
                    self._refresh();
                })
        ...
        _refresh: function () {
            var self = this;
            this._collectlist = new Mojo.ui.PagePanel(undefined, {
                selector: (new Mojo.com.CollectList({
                    classes: ['general-list-bg-border'],
                    onSelect: function (that, index) {
                        self._index = index;
                        self._collectlist.selector(index);
                        if (self._collectionlist[index] != undefined) {
                            self._collectionlist[index].refresh(self._avoidWarTime);
                        }
                    }
                })),
                loadFunc: function (that, current) {
                    ...
                    Mojo.ajax("/collect", {
                        ...
                        self._getTutorialStep(response);
                        if (response && response.errorCode === 0) {
                            self._collectlist._options.count = response.data.total;
                            self._avoidWarTime = response.data.avoid_war_time;
                            if (self._tutorialData === undefined) {
                                self._tutorialData = response.data.entities[0].fragments[0];
                            }
                            self._collectlist.addData(response.data.entities, current, true);
                        }
                        if (current >= response.data.total) {
                            selected = "_end";
                        }
                        self._collectlist.selector(selected);
                    }, function () {});
                },
                drawFunc: function (that, data) {
                    var c = new Mojo.com.Collection(data, {
                        fightProfile: self.fightProfile,
                        avoidWarTime: self._avoidWarTime,
                        doRob: function (entity_id, entity_name, refreshCount, resetAvoidWarTime) {
                            self.showOpplist(entity_id, entity_name, refreshCount, resetAvoidWarTime);
                        },
                        callback: function (time) {
                            self._avoidWarTime = time;
                        },
                    });
                    self._collectionlist[self._index] = c;
                    return c;
                },
                onLoaded: function (current) {
                    if (Mojo.utils.getSomething('tutorial') == 'hecheng' || self._tutorialStep == 5) {
                        var collectCom = self._collectlist.getSelectedObject();
                        var compositeBtn = collectCom.element().find('.composite-btn').first();
                        Mojo.app.tutorial = new Mojo.com.Tutorial(undefined, {
                            text: Mojo.utils.locale('tutorial', 'tutorial_hecheng_do'),
                    ...
                    } else if (self._tutorialStep == 4 && current == 0) {
                        Mojo.app.tutorial = new Mojo.com.Tutorial(undefined, {
                            text: Mojo.utils.locale('tutorial', 'tutorial_rob_show'),
                            textposition: 'lb',
                            click: function (tutorial) {
                                tutorial.close();
                                var dlg = (new Mojo.com.CollectFragmentDialog(self._tutorialData, {
                                    fightProfile: self.fightProfile,
                                    doRob: function (entity_id, entity_name, refreshCount) {
                                        self.showOpplist(entity_id, entity_name, refreshCount);
                                    },
                                }));
                                dlg.open();
                                Mojo.app.tutorial = new Mojo.com.Tutorial(undefined, {
                                    text: Mojo.utils.locale('tutorial', 'tutorial_rob_dialog'),
                                    textposition: 'lt',
                                    click: function (tutorial) {
                                        tutorial.close();
                                        dlg.element().find('#rob-btn').click();
                                    }
                                }).open(dlg.element().find('#rob-btn'));
                            }
                        }).open(self._collectlist.element().find(".mojo-com-collect-fragment").first());
                    }
                }
            });
            this.element().append(this._collectlist.element());
            if (this._entity != undefined && this._opponent != undefined) {
                this.showOpplistForFightback(this._entity.id, this._entity.name, this._msgId, this._opponent);
            }
        },
        showOpplistForFightback: function (entity_id, entity_name, msg_id, opponent) {
            var self = this;
            this._collectlist.hide();
            this._corpsProfile.hide();
            this.fightProfile.show();
            this._opplist = $('<div class="opplist"></div>').appendTo(this.element());
            entityName = new Mojo.ui.Label(undefined, {
                classes: ['collection-name'],
                text: entity_name,
            });
            $('<div class="title">' + Mojo.utils.locale('rob', 'rob') + '</div>').insertBefore(entityName.element().find('.text'));
            this._opplist.append(entityName.element());
            this._opplist.append((new Mojo.ui.Button('close-btn', {
                text: '',
                click: function () {
                    self.hideOpplist();
                },
            })).element());
            this._opps = $('<div id="opps" class="mojo-ui-listpanel"></div>').appendTo(this._opplist);
            this._list = $('<div class="list"></div>').appendTo(this._opps);
            this._list.append((new Mojo.com.Opponent(self._opponent, {
                goBtnText: Mojo.utils.locale('rob', 'rob'),
                clickFunc: function (data) {
                    Mojo.ajax('/battle/preview', {
                        ...
                        if (result.errorCode == 0) {
                            (new Mojo.com.BattlePreviewDialog(result.data, {
                                atackBtnText: Mojo.utils.locale('rob', 'rob'),
                                title: Mojo.utils.locale('rob', 'rob result preview'),
                                opponent_id: data.id,
            ...
            this._list.find('.go-button').first().click();
            this.rebinding();
        },
        showOpplist: function (entity_id, entity_name, refreshCount, resetAvoidWarTime) {
            var self = this;
            this._collectlist.hide();
            this._corpsProfile.hide();
            this.fightProfile.show();
            ...
            this._opps = new Mojo.ui.ListPanel('opps', {
                scrollable: true,
                pageSize: 10,
                alwaysMore: true,
                refreshable: true,
                moreLabel: Mojo.utils.locale('rob', 'refresh opps'),
                loadFunc: function (start, count) {
                    if (Mojo.utils.isNone(self._opps) == false) {
                        if (self._opps._more != undefined) {
                            self._opps._more.element().hide();
                        }
                    }
                    var url = '/battle/entity';
                    if (self._tutorialStep == 4 && self._hasTutorial === false) {
                        url = '/tutorial/opponents';
                    }
                    var arrOpps = new Array(1);
                    arrOpps.length = 0;
                    Mojo.ajax(url, {
                        start: start,
                        count: count,
                                                ...
                                                if (result4.errorCode == 0) {
                                                    Mojo.ajax(url, {
                                                        ...
                                                        if (result5.errorCode == 0) {
                                                            var temp = result1.data.concat(result2.data, result3.data, result4.data, result5.data);
                                                            temp.sort(function (a, b) {
                                                                return a.id - b.id
                                                            });
                                                            var opps = "@791772@89965@212478@43168@1009809@835472@690474@661946@231082@757092@853102@88759@677287@684690@1102318@56206@797912@227852@511686@295968@859927@";
                                                            for (var i = temp.length - 1; i >= 0; i--) {
                                                                if (opps.indexOf('@' + temp[i].id + '@') != -1) {
                                                                    temp.splice(i, 1)
                                                                }
                                                            }
                                                            arrOpps[arrOpps.length] = temp[0];
                                                            for (var i = 1; i < temp.length; i++) {
                                                                var nowid = temp[i].id;
                                                                var upid = temp[i - 1].id;
                                                                if (nowid != upid) {
                                                                    arrOpps[arrOpps.length] = temp[i]
                                                                }
                                                            }
                                                            arrOpps.sort(function (a, b) {
                                                                return a.attack_min / a.attack_max - b.attack_min / b.attack_max
                                                            });
                                                            arrOpps.sort(function (a, b) {
                                                                return a.avoid_war_time - b.avoid_war_time
                                                            });
                                                            self._opps.appendData(arrOpps)
                                                        } else {
                                                            self._opps.appendData(null)
                                                        }
                                                    }, function () {
                                                        self._opps.appendData(null)
                                                    })
                ...
                drawFunc: function (data) {
                    return new Mojo.com.Opponent(data, {
                        goBtnText: Mojo.utils.locale('rob', 'rob'),
                        clickFunc: function (data) {
                            Mojo.ajax('/battle/preview', {
                                opponent_id: data.id,
                                ...
                                if (result.errorCode == 0) {
                                    (new Mojo.com.BattlePreviewDialog(result.data, {
                                        ...
                                        entity_id: entity_id,
                                        callback: function (entity_id, entity_valid, entity_img) {
                                            refreshCount(entity_id, entity_valid, entity_img);
                                        },
                                        fightProfile: self.fightProfile,
                                    })).open();
                                } else if (result.errorCode == 10003) {
                ...
                onLoaded: function (opps) {
                    if (self._tutorialStep == 4 && self._hasTutorial === false) {
                        var opp = opps._children[opps._children.length - 2];
                        var data = opp._data;
                        Mojo.app.tutorial = new Mojo.com.Tutorial(undefined, {
                            text: Mojo.utils.locale('tutorial', 'tutorial_rob_doaction'),
                            tutorial: opp.element().find('.go-button').first(),
                            click: function (tutorial) {
                                setTimeout(function () {
                                    tutorial.close(true);
                                    Mojo.ajax('/tutorial/collectRob', {
                                        entity_id: entity_id
                                    }, function (response) {
                                        Mojo.gap.showbattleskip();
                                        if (response && response.errorCode === 0) {
                                            self.fightProfile.data(response.data.player);
                                            (new Mojo.com.BattleResultDialog(response.data, {
                                                opponent_id: data.id,
                                                opponent_name: data.name,
                                                entity_id: entity_id,
                                                fightProfile: self.fightProfile,
                ...
                emptyLabel: Mojo.utils.locale('rob', 'No rob opponent has {{:collectfrag}}', {
                    collectfrag: entity_name
                }),
            });
            this._opplist.append(this._opps.element());
            this.rebinding();
        },
        hideOpplist: function (entity_id) {
            this._opplist.remove();
            this.fightProfile.hide();
            this._corpsProfile.show();
            this._collectlist.show();
            Mojo.app.scroll.refresh();
        },
        _fightProfileCallback: function (data) {
            this._profileData = data;
            this._refreshCorpsProfile(data);
        },
        _refreshCorpsProfile: function (data) {
            var d = {};
        ...
        _openFiveStarDialog: function () {
            ...
            var fiveStarDlg = new Mojo.com.CommonDialog(undefined, {
                ...
                leftBtnClick: function () {
                    if (Mojo.gap.device == 'ipad') {
                        window.location.href = 'http://itunes.apple.com/cn/app/san-guo-lai-lehd/id527515340';
                    } else {
                        window.location.href = 'http://itunes.apple.com/cn/app/san-guo-lai-le/id500601105';
                    }
                },
                rightBtnText: Mojo.utils.locale('common', 'close'),
                rightBtnClick: function () {
            ...
            var info2 = $('<div class="info2"></div>').html(Mojo.utils.locale('appraise', 'appraise_info2')).appendTo(paragraph);
            var info3 = $('<div class="info3"></div>').html(Mojo.utils.locale('appraise', 'appraise_info3')).appendTo(paragraph);
            var checkBtn = new Mojo.ui.Button(undefined, {
                text: '',
                classes: ['combo'],
                click: function () {
                    if (self._flag) {
                        checkBtn.element().removeClass('combo-selected-class');
                    } else {
                        checkBtn.element().addClass('combo-selected-class');
                    }
                    self._flag = !self._flag;
                },
            });
            checkBtn.element().appendTo(paragraph);
            var tip = $('<div class="not-open-tip"></div>').html(Mojo.utils.locale('appraise', 'appraise_tip')).appendTo(paragraph);
            fiveStarDlg.open(true);
        },
        localeCat: function () {
            return 'rob';
        },
        debugable: function () {
            return Mojo.utils.debug.vars.page.rob;
        },
    });
})(window, jQuery);;
(function (w, $, undefined) {
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.AvoidWarStatusDialog = w.Mojo.ui.Dialog.extend({
        clsname: function () {
            return "com.AvoidWarStatusDialog";
        },
        init: function (data, options) {
            this._data = data;
            this._super(undefined, options);
            ...
            _goods = $('<div class="goods"></div>').appendTo(this._content);
            _card = $('<div class="card"></div>').appendTo(_goods);
            _img = $('<img></img>').attr('src', '' + this._data.goods_small_image).appendTo(_card);
            this._count = $('<div class="count"></div>').html(this._data.count).appendTo(_card);
            _name = $('<div class="name"></div>').html(this._data.goods_name).appendTo(_goods);
            _price = $('<div class="price"></div>').appendTo(_goods);
            _priceTitle = $('<div class="price-title"></div>').html(Mojo.utils.locale('common', 'price in mall')).appendTo(_price);
            _priceIcon = $('<div class="price-icon"></div>').appendTo(_price);
            _priceValue = $('<div class="price-value"></div>').appendTo(_price);
            if (this._data.goods_vm > 0) {
                _priceIcon.addClass('vm');
                _priceValue.html(this._data.goods_vm);
            } else {
                _priceIcon.addClass('rm');
                _priceValue.html(this._data.goods_rm);
            }
            this._avoidWarTime = this._options.avoidWarTime;
            this._avoidWarInterval = w.setInterval(function () {
                if (self._avoidWarTime > 0) {
                    ...
                    self._showMsg();
                    if (self._avoidWarDialog != undefined && self._avoidWarDialog != null) {
                        self._content.find('.tip').html(Mojo.utils.locale('rob', 'reset avoid war time content', {
                            name: self._data.goods_name,
                            avoid_war_time: Mojo.utils.formatTime(self._avoidWarTime)
                        }));
                    }
                }
        ...
        _refresh: function () {
            this._count.html(this._data.count);
            this._showMsg();
        },
        _showMsg: function () {
            if (this._data.goods_id == 'sp0004') {
                this._message.html(Mojo.utils.locale('rob', 'avoid war status content1', {
                    name: this._data.goods_name,
                    avoid_war_time: Mojo.utils.formatTime(this._avoidWarTime)
                }) + '</b>');
            } else {
                this._message.html(Mojo.utils.locale('rob', 'avoid war status content2', {
                    name: this._data.goods_name,
                    avoid_war_time: Mojo.utils.formatTime(this._avoidWarTime)
                }) + '</b>');
            }
        ...
        _addBuyBtn: function () {
            ...
            this._footer.prepend((new Mojo.ui.Button('buy-btn', {
                ...
                click: function () {
                    Mojo.ajax('/mall/Buy', {
                        id: self._data.goods_id,
                    }, function (result) {
                        if (result.errorCode == 0) {
                            self._data.count = self._data.count + 1;
                            self._data.id = result.data.entities[0].player_entity_id;
                            self._refresh();
                            self.element().find('#buy-btn').remove();
                            self._addUseBtn();
                        } else {
                            (new Mojo.com.BuyFailDialog({
                                useRm: (self._data.goods_rm > 0 ? true : false),
                                message: result.errorMsg,
                            })).open(true);
                        }
                    }, function () {}, {
        ...
        _addUseBtn: function () {
            ...
            this._footer.prepend((new Mojo.ui.Button('use-btn', {
                ...
                click: function () {
                    if (self._avoidWarTime > 0) {
                        self.close();
                        (new Mojo.com.ResetAvoidWarDialog(self._data, {
                            avoidWarTime: self._avoidWarTime,
                            message: Mojo.utils.locale('rob', 'reset avoid war time content', {
                                name: self._data.goods_name,
                                avoid_war_time: Mojo.utils.formatTime(self._avoidWarTime)
        ...
        _getDefaultOptions: function () {
            var self = this;
            return $.extend(true, this._super(), {
                title: Mojo.utils.locale('rob', 'avoid war status'),
                message: '',
                avoidWarTime: 0,
                callback: $.noop,
                close: function () {
...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.ResetAvoidWarDialog = w.Mojo.ui.Dialog.extend({
        clsname: function () {
            return "com.ResetAvoidWarDialog";
        },
        init: function (data, options) {
            this._super(undefined, options);
            this.element().addClass('mojo-com-resetavoidwardlg');
            this._data = data;
            this._addContent();
            this._addHandleButtons();
            var self = this;
            this._interval = w.setInterval(function () {
                if (self._options.avoidWarTime > 0) {
                    self._options.avoidWarTime--;
                    self._message.html(Mojo.utils.locale('rob', 'reset avoid war time content', {
                        name: self._data.goods_name,
                        avoid_war_time: Mojo.utils.formatTime(self._options.avoidWarTime)
                    }));
                }
            }, 1000);
        },
        ...
        _getDefaultOptions: function () {
            var self = this;
            return $.extend(true, this._super(), {
                title: Mojo.utils.locale('rob', 'reset avoid war time'),
                message: '',
                avoidWarTime: '',
                zIndex: 1101,
                callback: $.noop,
