...
(function (w, $, undefined) {
    w.Mojo = w.Mojo || {};
    w.Mojo.com = w.Mojo.com || {};
    w.Mojo.com.PropsElement = w.Mojo.Object.extend({
        clsname: function () {
        ...
        _couldUse: function () {
            ...
            if ((this._data.id == 'd01' || this._data.id == 'd02') && this._options.baseProfile.data().ep >= this._options.baseProfile.data().energy) {
                return false;
            } else if (this._data.id == 'd03' && this._options.baseProfile.data().sp >= this._options.baseProfile.data().stamina) {
                return false;
            }
            return true;
        },
        ...
        _setCard: function () {
            ...
            new Mojo.com.SmallEntity(this._data, {
                callback: function (card) {
                    self.element().find(".card").append(card.element());
                },
                click: function () {
                    var entitydetailDialog = new Mojo.ui.Dialog(undefined, {
                        title: Mojo.utils.locale('props', 'props_detail_title'),
                        content: (new Mojo.com.LargeEntity(self._data)).element()
                    });
                    var goods = {
                        name: self._data.name
                    };
                    $.extend(true, goods, self._data.goods);
                    new Mojo.com.Price(goods, {
                        showTitle: true
                    }).element().addClass("price-tip").insertBefore(entitydetailDialog._footer);
                    if (self._data.trade_type != 0) {
                        (new Mojo.ui.Button(undefined, {
                            text: Mojo.utils.locale('common', 'buy'),
                            special: "button-big-red",
                            click: function () {
                                self._buyProps(function (response) {
                                    entitydetailDialog.close();
                                    if (response && response.errorCode === 0) {
                                        if (self._options.onBuyProps instanceof Function) {
                                            self._options.onBuyProps(response);
                                        }
                                    }
                                });
                            }
                    ...
                    if (self._data.use_type == 1) {
                        var usebtn = (new Mojo.ui.Button(undefined, {
                            text: Mojo.utils.locale('common', 'use'),
                            special: 'button-big-blue',
                            disableClick: function () {
                                Mojo.app.toast.show(self._whyCouldNotUse());
                            },
                            click: function () {
                                self._checkAvoidWarTime(function (isUse) {
                                    if (isUse === true) {
                                        self._useProps(function (response) {
                                            entitydetailDialog.close();
                                            if (response && response.errorCode === 0) {
                                                if (self._options.onUseProps instanceof Function) {
                                                    self._options.onUseProps(response.data);
                                                }
                                            }
                                        });
                                    }
                        ...
                        if (self._couldUse() === false) {
                            usebtn.disable(true);
                        }
                        usebtn.element().addClass("buy-props").appendTo(entitydetailDialog._footer);
                    }
                    if (self._data.id == 'd12') {
                        (new Mojo.ui.Button(undefined, {
                    ...
                    if (parseInt(self._data.use_type) == 2) {
                        (new Mojo.ui.Button(undefined, {
                            text: Mojo.utils.locale('common', 'close'),
                            click: function () {
                                entitydetailDialog.close();
        ...
        _setCount: function () {
            if (this._countDiv === undefined) {
                this._countDiv = $('<div></div>').addClass("count").html((this._data.count ? this._data.count : "0")).appendTo(this.element().find(".card"));
            } else {
        ...
        _setDetail: function () {
            $('<div></div>').addClass("name").html(this._data.name).appendTo(this.element().find(".info"));
            $('<div></div>').addClass("description").html(this._data.description).appendTo(this.element().find(".info"));
        },
        _setPrice: function () {
        ...
        _setAdvertising: function () {
            if (this._data.goods && this._data.goods.discount < 100) {
                $('<div class="goods-advertising"></div>').addClass("discount-goods-flag").html(Mojo.utils.locale("mall", 'discount_goods_flag_new', {
        ...
        _setButtons: function () {
            ...
            this._tradeBtn = (new Mojo.ui.Button(undefined, {
                text: Mojo.utils.locale('common', 'buy'),
                special: "button-big-red",
                classes: ['align-right', ],
                click: function () {
                    var buyWarningDialog = new Mojo.ui.Dialog(undefined, {
                        ...
                        content: $('<div class="tip">' + Mojo.utils.locale('props', 'buy_warning_content') + '</div>'),
                        deviceaware: true
                    });
                    new Mojo.ui.Button(undefined, {
        ...
        _getDefaultOptions: function () {
            return {
                ...
                onUseProps: undefined,
                onBuyProps: undefined,
                propsPage: undefined
            };
        },
        _checkAvoidWarTime: function (callback) {
            var self = this;
            if (this._data.id == "d04" || this._data.id == "d05") {
                var avoidWarTime = this._options.propsPage.avoidWarTime();
                var pageLoadedTime = this._options.propsPage.pageLoadedTime();
                var pageOpenedTime = ((new Date()).getTime() - pageLoadedTime) / 1000;
                if (avoidWarTime > 0 && pageOpenedTime < avoidWarTime) {
                    var avoidWarTimeSureDialog = new Mojo.ui.Dialog(undefined, {
                        title: Mojo.utils.locale('props', "sure_avoid_war_time_title"),
                        content: $('<div class="tip"></div>').html(Mojo.utils.locale('props', 'sure_avoid_war_time_content').replace(/\{\{name\}\}/g, this._data.name)),
                        deviceaware: true,
                        close: function () {
                            self._options.propsPage.stopUpdateAvoidWarTime();
                        }
                    });
                    var avoidWarTimeDiv = $('<div class="tip avoid-war-time"></div>').html(Mojo.utils.formatTimePlus(Mojo.utils.locale('props', 'avoid_war_time'), (avoidWarTime - pageOpenedTime))).appendTo(avoidWarTimeSureDialog._content);
                    new Mojo.ui.Button(undefined, {
                        text: Mojo.utils.locale('common', 'use'),
                        special: "button-big-red",
                        click: function () {
                            if (callback instanceof Function) {
                                avoidWarTimeSureDialog.close();
                                callback(true);
                            }
                        }
                    }).element().appendTo(avoidWarTimeSureDialog._footer);
                    new Mojo.ui.Button(undefined, {
                        text: Mojo.utils.locale('common', 'close'),
                        special: "button-big-blue",
                        click: function () {
                    ...
                    avoidWarTimeSureDialog.open(true);
                    this._options.propsPage.startUpdateAvoidWarTime(avoidWarTimeDiv);
        ...
        _buyProps: function (callback) {
            var self = this;
            Mojo.ajax("/mall/buy", {
                id: this._data.goods_id
            }, function (response) {
                if (response && response.errorCode === 0) {
                    self._data.count = parseInt(self._data.count || 0) + 1;
                    self._setCount();
                    var entity = response.data.entities[0];
                    if (entity != undefined && self._data.player_entity_id === undefined) {
                        self._data.player_entity_id = entity.player_entity_id;
                    }
        ...
        _useProps: function (callback) {
            var self = this;
            Mojo.ajax("/entity/use", {
                id: self._data.player_entity_id
            }, function (response) {
                if (response && response.errorCode === 0) {
                    ...
                    var resultContent = "";
                    switch (parseInt(response.data.use_rule)) {
                    case 1:
                        resultContent = Mojo.utils.locale('props', 'add_ep').replace(/\{\{name\}\}/g, self._data.name) + result.value;
                        break;
                    case 2:
                        resultContent = Mojo.utils.locale('props', 'add_sp').replace(/\{\{name\}\}/g, self._data.name) + result.value;
                        break;
                    case 3:
                        var avoidWarTime = self._options.propsPage.avoidWarTime();
                        var pageLoadedTime = self._options.propsPage.pageLoadedTime();
                        var pageOpenedTime = ((new Date()).getTime() - pageLoadedTime) / 1000;
                        if (avoidWarTime > 0 && pageOpenedTime < avoidWarTime) {
                            resultContent = Mojo.utils.formatTimePlus(Mojo.utils.locale('props', 'add_avoid_war').replace(/\{\{name\}\}/g, self._data.name), parseInt(result.value));
                        } else {
                            resultContent = Mojo.utils.formatTimePlus(Mojo.utils.locale('props', 'first_add_avoid_war').replace(/\{\{name\}\}/g, self._data.name), parseInt(result.value));
                        }
                        self._options.propsPage.avoidWarTime(parseInt(result.value));
                        break;
                    case 4:
                        resultContent = Mojo.utils.locale('props', 'add_vm').replace(/\{\{name\}\}/g, self._data.name) + result.value;
                        break;
                    default:
                        resultContent = Mojo.utils.locale('props', 'add_null');
                        break;
                    }
                    Mojo.app.toast.show(resultContent);
                }
                if (callback instanceof Function) {
                    callback(response);
                }
...
(function (w, $, undefined) {
    ...
    w.Mojo.page.Mall = w.Mojo.Page.extend({
        clsname: function () {
            return "page.Mall";
        },
        init: function () {
            ...
            this._paymentList = undefined;
            this._paymentAdvertising = undefined;
            this._paymentButtonFlag = undefined;
            this._tavern = undefined;
            this._tavernUpdateTime = undefined;
            this._tavernUpdateTimeDiv = undefined;
            this._hasUpdateTavernTime = false;
            this._avoidWarTime = 0;
            this._pageLoadedTime = undefined;
            this._isUpdateAvoidWarTime = false;
            this._updateAvoidWarTimeHandle = undefined;
            var self = this;
            Mojo.gap.rechargeCallback = function () {
                if (Mojo.utils.isNone(self._paymentList) == false) {
                    self._paymentAdvertising = undefined;
                    self._paymentList.removeAll();
                    self._paymentList._load();
                }
                self.baseProfile.sync();
            };
            Mojo.ajax("/mall/type", {}, function (response) {
                self._panels = [];
                if (response && response.errorCode === 0) {
                    for (var index in response.data) {
                        var goodsType = response.data[index];
                        if (goodsType.id > 0) {
                            self._panels.push({
                                id: goodsType.id,
                                name: goodsType.name,
                        ...
                        } else if (goodsType.id == -1) {
                            self._paymentButtonFlag = goodsType.status;
                        }
                    }
                } else {
                    self._panels = [];
        ...
        _createPaymentButton: function () {
            ...
            var paymentButtonFlag = this._paymentButtonFlag || Mojo.utils.getSomething("pstatus");
            if (Mojo.utils.isNone(paymentButtonFlag) == false) {
                this._paymentButton.setFlag({
                    text: Mojo.utils.locale('common', paymentButtonFlag),
                    classes: ["flag-" + paymentButtonFlag]
                });
            }
        ...
        _addMallTabs: function () {
            ...
            this._mallTabs = new Mojo.ui.Tabs('tabs-mall', {
                selectionChange: function (index) {
                    ...
                    if (index == 2 && self._needCreate == true) needcreate = true;
                    self._createMallList(index, needcreate);
                    self._selected = index;
                }
            });
            $.each(this._panels, function (i, p) {
                var status = p.status || Mojo.utils.getSomething("tstatus");
                if (Mojo.utils.isNone(status) == false) {
                    self._mallTabs.addTab(p.name, undefined, {
                        text: Mojo.utils.locale('common', status),
                        classes: ["flag-" + status]
                    });
                } else {
                    self._mallTabs.addTab(p.name);
                }
            });
            this.element().append(this._mallTabs.element());
            var selected = 1;
            if (Mojo.utils.getSomething("selected")) {
                selected = Mojo.utils.getSomething("selected");
            }
            if (selected < 0 || selected >= this._panels.length) {
                selected = 1;
                this._createPaymentDialog();
            }
            this._mallTabs._select(selected);
            this._selected = selected;
        },
        _createMallList: function (index, needcreate) {
            var self = this;
            var p = this._panels[index];
            if (needcreate) {
                p.malllist = undefined;
            }
            self._needCreate = false;
            if (p.malllist === undefined || p.malllist === null) {
                p.index = index;
                if (p.id == "payment") {
                    self._createPaymentList(p);
                } else {
                    self._createGoodsList(p);
                }
                this._mallTabs.setPanel(index, p.malllist);
                this._mallTabs.element().find(".panel").addClass("box-outter");
            }
        },
        _createGeneralList: function (p) {
            var self = this;
            p.malllist = new Mojo.ui.ListPanel('mall-list-' + p.id, {
                scrollable: true,
                showMore: false,
                pageSize: 20,
                loadFunc: function (count, pagesize, params) {
                    Mojo.ajax("/mall", {
                        ...
                        if (response && response.errorCode === 0) {
                            if (Mojo.utils.isNone(response.data.time) == false && self._tavernUpdateTime == undefined) {
                                self._tavernUpdateTime = response.data.time;
                                self._tavern = p;
                            }
                            var goodsData = [];
                            for (var index in response.data.list) {
                                var goods = response.data.list[index];
                                for (var gindex in goods.entities) {
                                    var entity = goods.entities[gindex];
                                    goodsData.push({
                                        'id': goods.id,
                                        'name': goods.name,
                            ...
                            p.malllist.appendData(goodsData);
                        } else {
                            p.malllist.appendData(null);
                        }
                    }, function () {
                        p.malllist.appendData(null);
                    });
                ...
                drawFunc: function (data) {
                    return (new Mojo.com.Goods(data, {
                        getPlayerVm: function () {
                            return self.baseProfile._data.vm;
                        },
                        onBuy: function (count) {
                            self._buyGoods(data, undefined, count);
                        },
                        click: function () {
                            var entityDetailDialog = new Mojo.ui.Dialog(undefined, {
                ...
                onLoaded: function (that) {
                    if (Mojo.utils.isNone(self._tavernUpdateTime) == false && self._hasUpdateTavernTime == false) {
                        that.element().addClass("tavern-listpanel");
                        self._updateTavernTime(that);
                    }
                }
            });
        },
        _createPropsList: function (p) {
            var self = this;
            p.malllist = new Mojo.ui.ListPanel('mall-list-' + p.id, {
                scrollable: true,
                pageSize: 20,
                showMore: false,
                loadFunc: function () {
                    Mojo.ajax("/entity/props", {}, function (response) {
                        if (self._pageLoadedTime === undefined) {
                            self._pageLoadedTime = (new Date()).getTime();
                        }
                        if (response && response.errorCode === 0) {
                            if (response.data.avoid_war_time) {
                                self._avoidWarTime = parseInt(response.data.avoid_war_time);
                            }
                            self.baseProfile.data(response.data.player);
                            p.malllist.appendData(response.data.list);
                        } else {
                            p.malllist.appendData(null);
                        }
                    }, function () {
                ...
                drawFunc: function (data) {
                    return (new Mojo.com.PropsElement(data, {
                        baseProfile: self.baseProfile,
                        propsPage: self,
                        onUseProps: function (result) {
                            self._onUseProps(p, result, data);
                        },
                        onBuyProps: function (result) {
                            Mojo.track.onEvent('15_011');
                            self._onBuyProps(p, data, result);
                        }
                    }));
                }
            });
        },
        avoidWarTime: function (t) {
            if (t != undefined && t != null) {
                this._avoidWarTime = t;
                this._pageLoadedTime = (new Date()).getTime();
            }
            return this._avoidWarTime;
        },
        startUpdateAvoidWarTime: function (el) {
            this._isUpdateAvoidWarTime = true;
            clearTimeout(this._updateAvoidWarTimeHandle);
            this._updateAvoidWarTime(el);
        },
        stopUpdateAvoidWarTime: function () {
            this._isUpdateAvoidWarTime = false;
            clearTimeout(this._updateAvoidWarTimeHandle);
        },
        _updateAvoidWarTime: function (el) {
            var pageOpenedTime = ((new Date()).getTime() - this._pageLoadedTime) / 1000;
            var avoidWarTime = this._avoidWarTime - pageOpenedTime;
            if (avoidWarTime < 0) {
                avoidWarTime = 0;
            }
            if (el instanceof jQuery) {
                el.html(Mojo.utils.formatTimePlus(Mojo.utils.locale('props', 'avoid_war_time'), avoidWarTime));
            }
            if (this._isUpdateAvoidWarTime === true) {
                var self = this;
                this._updateAvoidWarTimeHandle = setTimeout(function () {
                    self._updateAvoidWarTime(el);
                }, 1000);
            }
        },
        ...
        _onUseProps: function (p, result, data) {
            var goods = data.goods;
            if (Mojo.utils.isNone(result.player) == false) {
                this.baseProfile.data(result.player);
            } else {
                var player = this.baseProfile.data();
                if (parseInt(result.use_rule) == 1) {
                    player.ep = parseInt(player.ep) + parseInt(result.value);
                } else if (parseInt(result.use_rule) == 2) {
                    player.sp = parseInt(player.sp) + parseInt(result.value);
                } else if (parseInt(result.use_rule) == 4) {
                    player.vm = parseInt(player.vm) + parseInt(result.value);
                }
                this.baseProfile.data(player);
            }
            this._refreshProps(p);
            Mojo.track.onSell(goods.id, 1, "使用", true);
        },
        _onBuyProps: function (p, data, response) {
            if (Mojo.utils.isNone(response) == false && response.errorCode === 0) {
                var goods = data.goods;
                var player = this.baseProfile.data();
                if (response.data && response.data.player) {
                    player.vm = parseInt(response.data.player.vm);
                    player.rm = parseInt(response.data.player.rm);
                } else {
                    if (goods.vm > 0) {
                        player.vm = parseInt(player.vm) - parseInt((goods.discount / 100) * goods.vm);
                    } else if (goods.rm > 0) {
                        player.rm = parseInt(player.rm) - parseInt((goods.discount / 100) * goods.rm);
                    }
                }
                this.baseProfile.data(player);
        ...
        _refreshProps: function (p) {
            var pels = p.malllist._children;
            for (var index in pels) {
                var pel = pels[index];
                pel._setButtonsDisable();
            }
        },
        _createGoodsList: function (p) {
            var self = this;
            if (p.id == 3) {
                this._createPropsList(p);
            } else {
                this._createGeneralList(p);
            }
        },
        _updateTavernTime: function (listpanel) {
            this._hasUpdateTavernTime = true;
            var formatStr = Mojo.utils.locale('mall', 'tavern_update_time');
            if (this._tavernUpdateTimeDiv === undefined) {
                this._tavernUpdateTimeDiv = $('<div class="tavern-time"></div>').html(Mojo.utils.formatTimePlus(formatStr, this._tavernUpdateTime - 1)).insertBefore(listpanel.element());
            }
            if (this._tavernUpdateTime && this._tavernUpdateTime > 0) {
                this._tavernUpdateTime -= 1;
                var self = this;
                self._tavernUpdateTimeDiv.html(Mojo.utils.formatTimePlus(formatStr, self._tavernUpdateTime));
                setTimeout(function () {
                    self._updateTavernTime();
                }, 1000);
            } else if (this._tavernUpdateTime <= 0 && Mojo.utils.isNone(this._tavern) == false) {
                this._tavern.malllist.element().remove();
                this._tavern.malllist = undefined;
                this._tavernUpdateTime = undefined;
                this._tavernUpdateTimeDiv = undefined;
                this._hasUpdateTavernTime = false;
        ...
        _createPaymentList: function (p) {
            var self = this;
            p.malllist = new Mojo.ui.ListPanel('payment-listpanel', {
                scrollable: true,
                showMore: false,
                pageSize: 20,
                loadFunc: function () {
                ...
                drawFunc: function (data) {
                    return (new Mojo.com.PaymentElement(data, {
                        click: function (p) {
                            Mojo.gap.purchase(self.baseProfile.data().id, p.id);
                        }
                    }));
                }
            });
        ...
        _refreshPlayerMoney: function (goods, count) {
            var player = this.baseProfile.data();
            if (count != undefined) {
                if (goods.vm > 0) {
                    player.vm = parseInt(player.vm) - parseInt((goods.discount / 100) * goods.vm * count);
                } else if (goods.rm > 0) {
                    player.rm = parseInt(player.rm) - parseInt((goods.discount / 100) * goods.rm * count);
                }
            } else {
                if (goods.vm > 0) {
                    player.vm = parseInt(player.vm) - parseInt((goods.discount / 100) * goods.vm);
        ...
        _buyGoods: function (goods, callback, count) {
            var self = this;
            var params = {};
            params['id'] = goods.id;
            params['type'] = goods.mallTypeId;
            if (count != undefined) {
                params['count'] = count;
            }
            Mojo.ajax("/mall/buy", params, function (response) {
                if (response && response.errorCode === 0) {
                    self._refreshPlayerMoney(goods, count);
                    if (self._selected == 0) {
