...
(function (w, $, undefined) {
    ...
    w.Mojo.page.Home = w.Mojo.Page.extend({
        ...
        fuben_auto: function(){
            ...
            Mojo.app.toast.show2("开始自动副本......");
            var auto_fuben_iv = setInterval(function(){
                if(fubens.length==0 && fubens_refresh.length==0){
                    fuben_refresh_count = fuben_refresh_count+1;
                    if(fuben_refresh_count > 15){
                        fuben_refresh_count = 0;
                ...
                }else if(fubens_refresh.length > 0){
                    Mojo.ajax('/fuben/fbTasks', {fuben_id:fubens_refresh[0].id,fuben_refresh:1}, function (result) {
                        if(result.errorCode == 0){
                            Mojo.app.toast.show2("[副本]["+fubens_refresh[0].name +"]重置成功");
                            var reset1 = fubens_refresh.splice(0,1);
                            reset1.name=result.data.cur_fuben.name;
                            fubens.push(reset1);
                        }else{
                            fubens_refresh.splice(0,1);
                        }
                    }, function () {}); 
                        ...
                        if(t_i < fb_tasks.length){
                            Mojo.ajax("/fuben/do",{id: fb_tasks[t_i].id},function(result){
                                ...
                                if(result.errorCode == 160003){
                                    ...
                                    }else if(result.data.fb_task.status==2){
